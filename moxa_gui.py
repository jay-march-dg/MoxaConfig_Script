...
DEFAULT_SCRIPT_NAME = "upload_moxa.py"
...
def derive_gateway(ip_address: str) -> str:
    network = ipaddress.ip_network(f"{ip_address}/255.255.255.0", strict=False)
...
def read_adapter_name_from_script_dir(script_path: Path) -> str:
    try:
        settings_path = script_path.parent / "adapter_settings.json"
        if not settings_path.exists():
            return ""
        data = json.loads(settings_path.read_text(encoding="utf-8"))
        return str(data.get("adapter_name", "")).strip()
    except Exception:
        return ""
...
class DeviceCsvModel(QAbstractTableModel):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.headers: list[str] = DEFAULT_HEADERS.copy()
    ...
    def load_csv(self, path: Path) -> None:
        ...
        if path.exists():
            with path.open("r", newline="", encoding="utf-8-sig") as handle:
                ...
                for required in DEFAULT_HEADERS:
    ...
    def validate_rows(self) -> list[str]:
        ...
        for idx, row in enumerate(self.rows, start=1):
            ...
            if not device_type:
                errors.append(f"Row {idx}: device_type is required.")
            ...
            if gateway and not is_valid_ipv4(gateway):
                errors.append(f"Row {idx}: invalid gateway '{gateway}'.")
    ...
    def _cell_invalid(self, row_index: int, column_name: str) -> bool:
        ...
        if column_name in {"device_name", "device_type", "ip_address"} and not value:
            return True

        if column_name == "ip_address" and value and not is_valid_ipv4(value):
            return True

        if column_name == "gateway" and value and not is_valid_ipv4(value):
            return True
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.settings = QSettings("LocalAutomation", "MoxaConfiguratorGUI")

        self.model = DeviceCsvModel(self)
    ...
    def _build_ui(self) -> None:
        self.setWindowTitle("Moxa Configurator")
    ...
    def _build_config_tab(self) -> QWidget:
        ...
        title = QLabel("Moxa Device Configurator")
        ...
        self.run_dry_button = QPushButton("Run Dry-Run")
        self.stop_button = QPushButton("Stop")
        self.stop_button.setObjectName("dangerButton")

        action_row = QHBoxLayout()
        action_row.addWidget(self.run_button)
        action_row.addWidget(self.run_dry_button)
        action_row.addWidget(self.stop_button)

        simple_hint = QLabel(
            "Use the device table to select what you want to configure. "
            "If you need to edit paths, templates, or page/action settings, go to Settings."
        )
        simple_hint.setWordWrap(True)
        simple_hint.setObjectName("subtitleLabel")

        right_col.addWidget(device_group)
        right_col.addLayout(action_row)
        right_col.addWidget(simple_hint)
        ...
        self.use_device_ip_check = QCheckBox("Use device IP for upload (--a2)")
        self.rdp_check = QCheckBox("RDP mode (--rdp)")
        self.skip_adapter_check = QCheckBox("Skip adapter change")
        self.debug_http_check = QCheckBox("Debug HTTP")
        self.default_dry_run_check = QCheckBox("Default to dry-run")
        ...
        flags_layout.addWidget(self.skip_adapter_check)
        flags_layout.addWidget(self.debug_http_check)
        flags_layout.addWidget(self.default_dry_run_check)
    ...
    def _build_settings_tab(self) -> QWidget:
        ...
        intro = QLabel(
            "All project paths and general run options live here. "
            "The command flags are available on the Configurator tab."
        )
        ...
        self.device_list_edit.setPlaceholderText(str(BASE_DIR / DEFAULT_DEVICE_LIST_NAME))
        browse_csv_btn = QPushButton("Browse...")
        reload_csv_btn = QPushButton("Reload CSV")

        self.template_dir_edit = QLineEdit()
        self.template_dir_edit.setPlaceholderText(str(BASE_DIR))
        browse_template_dir_btn = QPushButton("Browse...")

        self.template_file_edit = QLineEdit()
        self.template_file_edit.setPlaceholderText("Optional explicit template file")
        browse_template_file_btn = QPushButton("Browse...")
        ...
        row_csv_buttons = QHBoxLayout()
        row_csv_buttons.addWidget(browse_csv_btn)
        row_csv_buttons.addWidget(reload_csv_btn)
        csv_btn_holder = QWidget()
        csv_btn_holder.setLayout(row_csv_buttons)
        paths_layout.addWidget(csv_btn_holder, 1, 2)

        paths_layout.addWidget(QLabel("Template Dir"), 2, 0)
        paths_layout.addWidget(self.template_dir_edit, 2, 1)
        paths_layout.addWidget(browse_template_dir_btn, 2, 2)

        paths_layout.addWidget(QLabel("Template File"), 3, 0)
        paths_layout.addWidget(self.template_file_edit, 3, 1)
        paths_layout.addWidget(browse_template_file_btn, 3, 2)

        root.addWidget(paths_group)

        self.adapter_edit = QLineEdit()
        self.adapter_edit.setPlaceholderText("Ethernet")

        self.password_edit = QLineEdit()
        self.password_edit.setEchoMode(QLineEdit.EchoMode.Password)
        self.password_edit.setPlaceholderText("Moxa password (passed via environment)")

        self.scheme_combo = QComboBox()
        self.scheme_combo.addItems(["http", "https"])

        self.upload_page_edit = QLineEdit(DEFAULT_UPLOAD_PAGE)
        self.upload_action_edit = QLineEdit(DEFAULT_UPLOAD_ACTION)
        self.restart_page_edit = QLineEdit(DEFAULT_RESTART_PAGE)
        self.restart_action_edit = QLineEdit(DEFAULT_RESTART_ACTION)

        options_group = QGroupBox("Run Options")
        options_layout = QGridLayout(options_group)
        options_layout.addWidget(QLabel("Adapter Name"), 0, 0)
        options_layout.addWidget(self.adapter_edit, 0, 1)
        options_layout.addWidget(QLabel("Password"), 0, 2)
        options_layout.addWidget(self.password_edit, 0, 3)

        options_layout.addWidget(QLabel("Scheme"), 1, 0)
        options_layout.addWidget(self.scheme_combo, 1, 1)
        options_layout.addWidget(QLabel("Upload Page"), 1, 2)
        options_layout.addWidget(self.upload_page_edit, 1, 3)

        options_layout.addWidget(QLabel("Upload Action"), 2, 0)
        options_layout.addWidget(self.upload_action_edit, 2, 1)
        options_layout.addWidget(QLabel("Restart Page"), 2, 2)
        options_layout.addWidget(self.restart_page_edit, 2, 3)

        options_layout.addWidget(QLabel("Restart Action"), 3, 0)
        options_layout.addWidget(self.restart_action_edit, 3, 1)
        ...
        reload_csv_btn.clicked.connect(self.reload_csv)
        browse_template_dir_btn.clicked.connect(self.browse_template_dir)
        browse_template_file_btn.clicked.connect(self.browse_template_file)
    ...
    def _connect_signals(self) -> None:
        ...
        self.template_dir_edit.textChanged.connect(self.update_device_details)
        self.template_file_edit.textChanged.connect(self.update_device_details)
        ...
        self.run_button.clicked.connect(lambda: self.start_run(force_dry_run=False))
        self.run_dry_button.clicked.connect(lambda: self.start_run(force_dry_run=True))
    ...
    def _load_settings(self) -> None:
        ...
        self.device_list_edit.setText(self.settings.value("device_list_path", ""))
        self.template_dir_edit.setText(self.settings.value("template_dir", ""))
        self.template_file_edit.setText(self.settings.value("template_file", ""))
        self.adapter_edit.setText(self.settings.value("adapter_name", ""))
        self.password_edit.setText(self.settings.value("password", ""))
        self.scheme_combo.setCurrentText(self.settings.value("scheme", "http"))

        self.upload_page_edit.setText(self.settings.value("upload_page", DEFAULT_UPLOAD_PAGE))
        self.upload_action_edit.setText(self.settings.value("upload_action", DEFAULT_UPLOAD_ACTION))
        self.restart_page_edit.setText(self.settings.value("restart_page", DEFAULT_RESTART_PAGE))
        self.restart_action_edit.setText(self.settings.value("restart_action", DEFAULT_RESTART_ACTION))
        ...
        self.skip_adapter_check.setChecked(self.settings.value("skip_adapter", False, type=bool))
        self.debug_http_check.setChecked(self.settings.value("debug_http", False, type=bool))
        self.default_dry_run_check.setChecked(self.settings.value("default_dry_run", False, type=bool))
    ...
    def _fill_default_paths_if_blank(self) -> None:
        ...
        if not self.device_list_edit.text().strip():
            self.device_list_edit.setText(str(BASE_DIR / DEFAULT_DEVICE_LIST_NAME))

        if not self.template_dir_edit.text().strip():
            self.template_dir_edit.setText(str(BASE_DIR))

        script_path = clean_path(self.script_edit.text().strip())
        if not self.adapter_edit.text().strip() and script_path.exists():
            saved_adapter = read_adapter_name_from_script_dir(script_path)
            if saved_adapter:
                self.adapter_edit.setText(saved_adapter)
    ...
    def _save_settings(self) -> None:
        ...
        self.settings.setValue("device_list_path", self.device_list_edit.text().strip())
        self.settings.setValue("template_dir", self.template_dir_edit.text().strip())
        self.settings.setValue("template_file", self.template_file_edit.text().strip())
        self.settings.setValue("adapter_name", self.adapter_edit.text().strip())
        self.settings.setValue("password", self.password_edit.text())
        self.settings.setValue("scheme", self.scheme_combo.currentText())

        self.settings.setValue("upload_page", self.upload_page_edit.text().strip())
        self.settings.setValue("upload_action", self.upload_action_edit.text().strip())
        self.settings.setValue("restart_page", self.restart_page_edit.text().strip())
        self.settings.setValue("restart_action", self.restart_action_edit.text().strip())
        ...
        self.settings.setValue("skip_adapter", self.skip_adapter_check.isChecked())
        self.settings.setValue("debug_http", self.debug_http_check.isChecked())
        self.settings.setValue("default_dry_run", self.default_dry_run_check.isChecked())
        self.settings.setValue("selected_device", self.device_combo.currentText().strip())
        self.settings.setValue("geometry", self.saveGeometry())

    def _update_window_title(self) -> None:
        suffix = " *" if self.model.dirty else ""
        self.setWindowTitle(f"Moxa Configurator{suffix}")
    ...
    def _set_run_state(self, running: bool) -> None:
        ...
        self.run_dry_button.setEnabled(not running)
    ...
    def browse_script(self) -> None:
        ...
        script_path = clean_path(filename)

        if not self.template_dir_edit.text().strip():
            self.template_dir_edit.setText(str(script_path.parent))

        if not self.device_list_edit.text().strip():
            self.device_list_edit.setText(str(script_path.parent / "deviceList.csv"))
    ...
    def browse_csv(self) -> None:
        filename, _ = QFileDialog.getOpenFileName(
            self,
            "Select deviceList.csv",
            self.device_list_edit.text().strip() or str(BASE_DIR / DEFAULT_DEVICE_LIST_NAME),
            "CSV Files (*.csv);;All Files (*)",
        )
        if not filename:
            return

        self.device_list_edit.setText(filename)
        self.reload_csv()
    ...
    def browse_template_dir(self) -> None:
        directory = QFileDialog.getExistingDirectory(
            ...
            self.template_dir_edit.text().strip() or str(BASE_DIR),
        )
        if directory:
    ...
    def browse_template_file(self) -> None:
        filename, _ = QFileDialog.getOpenFileName(
            ...
            self.template_file_edit.text().strip() or self.template_dir_edit.text().strip() or str(BASE_DIR),
            "Text Files (*.txt);;All Files (*)",
        )
        if filename:
            self.template_file_edit.setText(filename)
    ...
    def reload_csv(self, silent: bool = False) -> bool:
        ...
        if not path_text:
            QMessageBox.warning(self, "CSV Required", "Please choose a deviceList.csv file first.")
            return False
    ...
    def update_device_details(self) -> None:
        ...
        self.type_value.setText(device_type or "-")
        ...
        template_file = self.template_file_edit.text().strip()
        if template_file:
            path = clean_path(template_file)
        else:
            template_dir_text = self.template_dir_edit.text().strip()
            if template_dir_text and device_type:
                path = clean_path(template_dir_text) / f"Moxa-Template-{device_type}.txt"
            else:
                path = None
    ...
    def build_process_args(self, dry_run: bool) -> tuple[list[str], QProcessEnvironment]:
        ...
        if device_row is None:
            raise ValueError(f"Device '{device_name}' was not found in the CSV.")

        csv_text = self.device_list_edit.text().strip()
        if not csv_text:
            raise ValueError("Device list CSV path is required.")

        csv_path = clean_path(csv_text)
        ...
        args: list[str] = [
            ...
            str(csv_path),
            "--scheme",
            self.scheme_combo.currentText().strip() or "http",
            "--upload-page",
            self.upload_page_edit.text().strip() or DEFAULT_UPLOAD_PAGE,
            "--upload-action",
            self.upload_action_edit.text().strip() or DEFAULT_UPLOAD_ACTION,
            "--restart-page",
            self.restart_page_edit.text().strip() or DEFAULT_RESTART_PAGE,
            "--restart-action",
            self.restart_action_edit.text().strip() or DEFAULT_RESTART_ACTION,
        ]

        if template_file_text:
            args.extend(["--template-file", str(clean_path(template_file_text))])
        else:
            template_dir = clean_path(template_dir_text) if template_dir_text else script_path.parent
            args.extend(["--template-dir", str(template_dir)])

        adapter_name = self.adapter_edit.text().strip()
        if adapter_name:
            args.extend(["--adapter-name", adapter_name])

        if self.use_device_ip_check.isChecked():
            args.append("--a2")
        if self.rdp_check.isChecked():
            args.append("--rdp")
        if self.skip_adapter_check.isChecked():
            args.append("--skip-adapter-change")
        if self.debug_http_check.isChecked():
            args.append("--debug-http")
        if dry_run:
            args.append("--dry-run")
        ...
        if password:
            env.insert("MOXA_PASSWORD", password)
    ...
    def start_run(self, force_dry_run: bool) -> None:
        ...
        dry_run = force_dry_run or self.default_dry_run_check.isChecked()

        try:
            args, env = self.build_process_args(dry_run=dry_run)
            script_path = clean_path(self.script_edit.text().strip())

            self.log_output.clear()
            self.append_log("=== Starting Moxa configurator ===\n")
    ...
    def on_process_finished(self, exit_code: int, exit_status: QProcess.ExitStatus) -> None:
        ...
        if exit_status == QProcess.ExitStatus.NormalExit and exit_code == 0:
            ...
            self.statusBar().showMessage("Configuration completed successfully.", 5000)
        elif exit_code == 130:
            self.append_log("\n=== Interrupted ===\n")
...
def main() -> int:
    ...
    app.setApplicationName("Moxa Configurator")
